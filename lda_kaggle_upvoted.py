...
def find_k(dictionary, doc_term_matrix, doc_clean, stop, start, step):
    ...
    for num_topics in range(start, stop, step):
        model = LdaMulticore(doc_term_matrix, num_topics=num_topics, id2word=dictionary, passes=2)
...
if __name__ == "__main__":
    ...
    '''
    # Apply LDA algorithm
    ldamodel = LdaMulticore(doc_term_matrix, num_topics = num_of_topics, id2word = dict, passes = 2)
    #vectorized_corpus = ldamodel[corpus_tfidf]
    #print(vectorized_corpus)
    #print(ldamodel.show_topics(num_topics=num_of_topics, num_words=words, log=False, formatted=True))
    topic_list = []
    for index, topic in ldamodel.show_topics(num_topics = num_of_topics, num_words = words, formatted = False):
        topic_list.append([w[0] for w in topic])
        print('Topic: {} \nWords: {}'.format(index, [w[0] for w in topic]))
    print(topic_list)
    # pyLDAvis Visualization
    pyLDAvis.enable_notebook()
    lda_visualization = pyLDAvis.gensim.prepare(ldamodel, doc_term_matrix, dict, sort_topics = False)
